...
# Need to run `npm run build` on wallet-server-ui
# Need to run `npm run build` on wallet-server-ui-proxy
...
cp -a ../common-ts/dist/. bin/common-ts

# Copy wallet-ts
mkdir -p bin/wallet-ts
cp -a ../wallet-ts/dist/. bin/wallet-ts
...
cp -a ../wallet-server-ui-proxy/dist/. bin/wallet-server-ui-proxy
# Replace config.json
